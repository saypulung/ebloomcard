...
class timthumb {
	...
	public function __construct(){
		//echo 'aaaaaaaaaaaaaaaaaa'.$this->param('src');
		//echo $_GET['src'];
		//echo $this->param('src');
	...
	protected function param($property, $default = ''){
		...
		if (isset ($_GET[$property])) {
			$my_url_image = 'http://'.$_SERVER['HTTP_HOST'].'/lelang/';
			$tts = '';
			if($property=='src'){
				$src = $_GET[$property];
				
				if($src=='no_img'){
					$tts = $my_url_image.'Pb_upl/No_Image.png';
					//break;
				}else
				if($src=='logo'){
					$tts = $my_url_image.'Pb_upl/logo.png';
					//break;
				}else{
					//src=a/aabbcc.png
					$e_url = explode('/',$_GET[$property]);
					$t_url = '';
					
					if($e_url[0]=='barang'){
						$t_url='Pb_upl/Images/Barang';
					}else
					if($e_url[0]=='konfirmasi'){
						$t_url='Pb_upl/Images/Konfirmasi';
					}
					if($e_url[0]=='youtube'){
						$t_url='http://img.youtube.com/vi/';
						return $t_url.$e_url[1].'/0.jpg';
					}
					return $my_url_image.$t_url.'/'.$e_url[1];
				}
